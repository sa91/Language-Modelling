...
class HiddenMarkovModel:
    ...
    def forward(self, I, A, B, O):
        ...
        alpha[0][:] = np.multiply(I, B[:][O[0]])
        C[0] = alpha[0][:].sum()
        alpha[0][:] = alpha[0][:] / C[0]
        for t in xrange(1, self.time_period):
            alpha[t][:] = np.multiply(np.matrix(alpha[t - 1][:])*np.matrix(A), B[:][O[t]])
            #print alpha[t][:]
            C[t] = alpha[t][:].sum()
            alpha[t][:] = alpha[t][:] / C[t]
        return alpha, C
    ...
    def backward(self, I, A, B, O, C=[]):
        ...
        for t in xrange(self.time_period - 2, -1, -1):
            temp = np.multiply(beta[t + 1][:], B[:][O[t + 1]])
            beta[t][:] = np.dot(A, temp.transpose()).transpose()/C[t]
        return beta

    def forward_backward(self, I, A, B, O, maxiters=100):
        oldLog = -float("inf")
        for iter in xrange(maxiters):
            ...
            for t in xrange(self.time_period - 1):
                ...
                #         gamma[t][i][j] = alpha[t][i]*A[i][j]*B[O[t+1]][j]*beta[t+1][j]
                gamma[t][:][:] /= gamma[t].sum()
                for i in xrange(self.state):
